...
public abstract class CourseCondition {
    ...
    public CourseCondition(ArrayList<Course> courses) throws OutOfCapacityException {
        if(courses.size() > this.getMAX_COURSES())  throw new OutOfCapacityException();
    ...
    public void addCourse(Course course) throws OutOfCapacityException, AlreadyExistException{
        if(this.courses.contains(course))   throw new AlreadyExistException();
        if(this.courses.size() >= this.getMAX_COURSES())    throw new OutOfCapacityException();
        this.courses.add(course);
    }

    /**
     * Remove the specified course. If the given course isn't in this list then the list wouldn't be changed and return false
     * @param course
     * To delete course
     * @return
